...
#
#
# def my_select_usermes(conn,uid):
#     cursor = conn.cursor()
#     now = datetime.datetime.now() - datetime.timedelta(days=3)
#     old_start_time = now.strftime('%Y-%m-%d 00:00:00')
#     # old_start_time = now.strftime('2019-04-19 00:00:00')
#     # old_end_time = now.strftime('2019-04-19 23:59:59')
#     old_end_time = now.strftime('%Y-%m-%d 23:59:59')
#     sql = "SELECT * FROM cr_dyonevideolist WHERE `create_time` >= '%s' AND `create_time` <= '%s' AND `uid` = '%s'" % (old_start_time, old_end_time, uid)
#     cursor.execute(sql)
#     data = cursor.fetchone()
#     return data
...
#     a = my_select_old_data(conn=conn,video_id='6673034519345368323')
#     print(a)
#     my_close(conn=conn)
...
def my_select_account(conn):
    cursor = conn.cursor()
    sql = 'SELECT account FROM cr_dy_fans_mes'
    cursor.execute(sql)
    datas = cursor.fetchall()
    return datas
...
if __name__ == '__main__':
    ...
    db = 'inshoot'
    conn = my_conn(host=host, user=user, passwd=passwd, port=port, db=db)
    my_select_account(conn=conn)
    my_close(conn=conn)
# print(wb_data.text)
# et = etree.HTML(wb_data.text)
# href = et.xpath('//*/a/@href')
# print(href)
